...
WORKFLOW_PATH = os.getenv("WORKFLOW_PATH", "/v1/chat-messages")
CONNECT_TIMEOUT = float(os.getenv("CONNECT_TIMEOUT", "10"))
READ_TIMEOUT = float(os.getenv("READ_TIMEOUT", "60"))
TERMINAL_EVENTS = [e.strip() for e in os.getenv("TERMINAL_EVENTS", "message_end,error").split(",") if e.strip()]
...
class DifyWorkflowUser(HttpUser):
    ...
    def test_workflow_stream(self) -> None:
        ...
        data = {
            ...
            "response_mode": "streaming",
            "user": f"user_{self.user_counter}",
            # "conversation_id": ""  # bỏ trống để tạo conversation mới mỗi request
        }
